...
- Consultant is **Денис Кузьмин** — `t.me/nirdest`, `linkedin.com/in/nirdest`, `github.com/nirdest`. All three are wired into the JSON-LD `Person` (`sameAs`) and the footer. No placeholders remain in any published file.
- **Telegram is the primary channel and appears in three places** (owner confirmed the handle on 2026-09-11): the `.profile-links` row in `#experience`, the footer, and `.m-alt` inside the lead modal. The modal one is the load-bearing copy: the modal puts `inert` on `#page`, so while it is open no other contact on the site can be reached, and someone who does not want to leave a contact and wait had no way out. It sits **below** the submit button on purpose — above it, it competes with the form.
